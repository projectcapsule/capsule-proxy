...
func main() {
	...
	bearerToken := flag.String("bearer-token", "", "Overwrite default kubernetes api token, needed for debug when running locally")
	...
	listenerOpts, err = options.NewKube(*k8sControlPlaneUrl, *capsuleUserGroup, *usernameClaimField, ctrl.GetConfigOrDie(), *bearerToken)
	if err != nil {
