...
@test "Nodes listing allowed" {
	[ "$(curl -s --cacert ~/.local/share/mkcert/rootCA.pem --cert ./hack/alice-oil.crt --key ./hack/alice-oil.key "https://127.0.0.1:9001/api/v1/nodes" | jq .items | jq length)" -eq "1" ]
	[ "$(curl -s --cacert ~/.local/share/mkcert/rootCA.pem --cert ./hack/bob-gas.crt --key ./hack/bob-gas.key "https://127.0.0.1:9001/api/v1/nodes" | jq .items | jq length)" -eq "0" ]
}
